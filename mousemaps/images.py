...
from mousemaps.datasets.annotations import (
...
def _remap_region_data(original_values, value_format, regionmapping, target_column, hemi=None):

    if hemi is not None:
        if hemi not in ["left", "right"]:
            raise ValueError(f"Invalid hemi value: {hemi}. Should be one of ['left', 'right']")
    ...
    if has_duplicated:
        if value_format == "matrix":
            raise ValueError("Duplicated regions in regionmapping, but annotation format is matrix")
        indices_dup = regionmapping_nona.groupby(target_column).apply(lambda x: x.index.tolist()).tolist()
        original_values_nona_dedup = np.array([
            np.mean(original_values_nona[indices, :], axis=0) for indices in indices_dup
        ])
        regions_nona_dedup = list(regionmapping_nona.groupby(target_column).groups.keys())
    else:
...
def load_region_data(annotation, return_original=False, remap_kws=None, data_dir=None, verbose=1):
    ...
    original_values = np.loadtxt(data_dir / "annotations" / annotation_full["rel_path"] / annotation_full["fname"], delimiter=",")
    regionmapping = pd.read_csv(data_dir / "annotations" / annotation_full["rel_path"] / annotation_full["regionmapping"])
    ...
    if annotation_full["format"] == "scalar":
        ...
        assert original_values.shape[0] == original_values.shape[1] == len(regionmapping)
    ...
    original_values_nona_dedup, region_nona_dedup = \
        _remap_region_data(original_values, annotation_full["format"], regionmapping, "allenccfv3_acronym", **remap_dict)
