...
from mousemaps.datasets.utils import (
    MOUSEMAPS_ANNOTS, MOUSEMAPS_ANNOTS_META,
...
def _match_annots_by_tuple(annot_tuple_list):
    ...
    for annot_tuple in annot_tuple_list:
        ...
        for annot in MOUSEMAPS_ANNOTS:
            curr_annot_tuple = tuple([annot[key] for key in ["source", "desc", "space", "res"]])
...
def fetch_annotation(annotations, data_dir=None, return_single=False, verbose=1):
    ...
    targ_fname_list = fetch_files(annotations_full, file_type="annotations", data_dir=data_dir, verbose=verbose)
...
def _fetch_annotation_meta_files(sources, which="regionmapping", data_dir=None, verbose=1):
    ...
    targ_fname_list = fetch_files(filtered_annot_meta_list, file_type="annotations-meta", data_dir=data_dir, verbose=verbose)
